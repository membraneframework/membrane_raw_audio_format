defmodule Membrane.Caps.Audio.Raw.FormatSpec do
  ...
  let :all_tuples, do: [
    {:s, 8, :any}, {:u, 8, :any},
    {:s, 16, :le}, {:u, 16, :le}, {:s, 16, :be}, {:u, 16, :be},
    {:s, 24, :le}, {:u, 24, :le}, {:s, 24, :be}, {:u, 24, :be},
    {:s, 32, :le}, {:u, 32, :le}, {:s, 32, :be}, {:u, 32, :be},
    {:f, 32, :le}, {:f, 32, :be}
  ]

  describe "to_tuple/1" do
    it "should split format atom to proper 3-element tuple" do
      Enum.zip(all_formats(), all_tuples()) |> Enum.each(fn {fmt, tuple} ->
  ...
  describe "from_tuple/1" do
    it "should join 3-element tuple to proper format atom" do
      Enum.zip(all_tuples(), all_formats()) |> Enum.each(fn {tuple, fmt} ->
